...
bool stream_state = 0; // This global bool is 0 when writing and 1 while reading

/*
 * Initializes class
 */
SWD_IF::SWD_IF(int SWDCLK, int SWDIO, int SWD_CLK_HALFPERIOD, bool debug){
  _SWDCLK = SWDCLK;
  _SWDIO = SWDIO;
 ...
 */
bool SWD_IF::DP_read(uint32_t& data, unsigned adress){
  uint8_t retry = 15;
  while(retry--){  
 ...
 * returns false if 16 tries to read fail
 */
bool SWD_IF::AP_read(uint32_t &data, unsigned address){
    uint8_t retry = 15;
    while(retry--){  
 ...
 */
bool SWD_IF::DP_write(uint32_t data, unsigned address){
    uint8_t retry = 15;
    while(retry--){  
 ...
 */
bool SWD_IF::AP_write(uint32_t data, unsigned address){
    uint8_t retry = 15;
    while(retry--){  
 ...
 */
void SWD_IF::write(uint32_t output_data, uint8_t length){
  if(stream_state == 1) {
    stream_state = trn(0);
  }
  while(length--){
 ...
 *This function reads LSB first as per SWD standard 
 *@length: number of bits to be read
 *returns read data
 */
uint32_t SWD_IF::read(uint8_t lenght){
  if(stream_state == 0) {
    stream_state = trn(1);
  }
  uint32_t temp = 0;
 ...
 * returns false if ACK response isn't OK
 */
bool SWD_IF::send_request(unsigned adrs, bool APnDP, bool RnW){
 ...
 * returns the input for a form of state-control
 */
bool SWD_IF::trn(bool RnW){
 ...
 */
bool SWD_IF::check_parity(uint32_t data_to_check){
